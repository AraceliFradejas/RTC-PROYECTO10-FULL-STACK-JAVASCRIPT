...
try {
  const { renderPage, getMetadata, metadataHtml, publicRoutes, siteUrl } = await server.ssrLoadModule('/src/entry-server.jsx');
  ...
  for (const route of [...publicRoutes, '/auth']) {
  ...
  console.log(`Prerendered ${publicRoutes.length + 1} routes, sitemap.xml and robots.txt.`);
} finally {
  await server.close();
}
