import { eventCategoryTones as categoryTones } from "../data/eventCategories.js";
...
export const EventCard = ({ event, horizontal = false, headingLevel = 3 }) => {
  ...
  return (
    <article
      className={`event-card event-card--${tone}${horizontal ? " event-card--horizontal" : ""}`}
    >
      <Link
        className="event-card__link"
        to={`/events/${event._id}`}
        aria-label={t("Ver {title}", { title: content.title })}
      >
        <div
          className={`event-card__visual${event.poster ? "" : " event-card__visual--empty"}`}
        >
          {event.poster ? (
            <img
              src={event.poster}
            ...
            <span className="event-card__monogram" aria-hidden="true">
              K.
            </span>
          )}
        </div>
        <div className="event-card__body">
          <div className="event-card__metadata">
            <time
              ...
              dateTime={event.date}
              aria-label={formatEventDate(event.date, language)}
            >
              <span aria-hidden="true" className="event-card__day">
                {new Intl.DateTimeFormat(locale, {
                  day: "2-digit",
                  timeZone: "Europe/Madrid",
                }).format(date)}
              </span>
              <span aria-hidden="true" className="event-card__month">
                {new Intl.DateTimeFormat(locale, {
                  month: "short",
                  timeZone: "Europe/Madrid",
                }).format(date)}
              </span>
              <span aria-hidden="true" className="event-card__year">
                {new Intl.DateTimeFormat(locale, {
                  year: "numeric",
                  timeZone: "Europe/Madrid",
                }).format(date)}
          ...
          {content.description && (
            <p className="event-card__description">{content.description}</p>
          )}
          <EventAvailability event={event} />
          <div className="event-card__footer">
            <span className="event-card__attendance">
              <Users aria-hidden="true" />{" "}
              {t(attendees === 1 ? "{count} asistente" : "{count} asistentes", {
                count: attendees,
              })}
            </span>
