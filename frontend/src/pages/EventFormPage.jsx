...
import { canEditEvent, localDateTime, eventFormData } from '../utils/eventEditing.js';
...
export const EventFormPage = () => {
  ...
  useEffect(() => {
    ...
    setFetching(true);
    getEvent(id, { signal: controller.signal }).then(({ data }) => {
      if (controller.signal.aborted) return;
      if (previewMode || !canEditEvent(data, user)) throw new Error('Solo la persona creadora puede modificar este evento.');
      setOriginal(data);
      setValues(Object.fromEntries(Object.keys(initial).map(key => [key, key === 'date' ? localDateTime(data.date) : data[key] ?? initial[key]])));
  ...
  const submit = async event => {
    ...
    try {
      ...
      const {
        data
      } = await apiRequest(editing ? `/events/${id}` : '/events', {
        method: editing ? 'PATCH' : 'POST',
        body: form,
        token, user
      });
      notify(editing ? 'Cambios guardados.' : 'Tu experiencia ya forma parte de la agenda KelseTS.');
  ...
  if (fetching) return <Loader full label={t('Preparando todos los detalles…')} />;
  if (loadError) return <section className="page shell"><h1>{t('No se puede editar esta charla')}</h1><p role="alert">{t(loadError)}</p><Link className="button button--dark" to={`/events/${id}`}>{t('Volver a la charla')}</Link></section>;
  return <section className="page page--narrow"><Link className="back-link" to={editing ? `/events/${id}` : "/events"}><ArrowLeft /> {t(editing ? "Volver a la charla" : "Volver a la agenda")}</Link><div className="form-heading"><p className="kicker">{t("Bring your story")}</p><h1>{t(editing ? "Editar experiencia" : "Crea una experiencia")}</h1><p>{t(editing ? "Actualiza los datos de tu charla. El cartel se conserva si no eliges otra imagen." : "Diseña una charla que ayude a otras personas a dar su siguiente paso.")}</p></div>
    <form className="event-form" onSubmit={submit} noValidate aria-busy={loading}><FormErrors errors={errors} prefix="event" />
      ...
      {editing && original?.poster && <img className="field--full" src={original.poster} alt={t("Cartel actual")} style={{ maxWidth: 180, borderRadius: 12 }} />}
      ...
      <button className="button button--accent button--wide field--full" disabled={loading}>{loading ? <><span className="mini-spinner" /> {t(editing ? "Guardando…" : "Publicando…")}</> : <>{t(editing ? "Guardar cambios" : "Publicar evento")} <Send /></>}</button>
    </form>
  </section>;
};
