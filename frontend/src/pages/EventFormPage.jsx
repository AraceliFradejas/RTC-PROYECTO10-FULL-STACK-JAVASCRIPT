...
const initial = { title: '', date: '', location: '', category: 'Liderazgo', capacity: 30, description: '' };
export const EventFormPage = () => {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      notify('Tu experiencia ya forma parte de la agenda KelseTS.'); navigate(`/events/${data._id}`);
    } catch (error) { setErrors({ form: error.message }); notify(error.message, 'error'); }
    finally { setLoading(false); }
  };
  return <section className="page page--narrow"><Link className="back-link" to="/events"><ArrowLeft /> Volver a la agenda</Link><div className="form-heading"><p className="kicker">Bring your story</p><h1>Crea una experiencia</h1><p>Diseña una charla que ayude a otras personas a dar su siguiente paso.</p></div>
    <form className="event-form" onSubmit={submit} noValidate>{errors.form && <div className="form-alert" role="alert">{errors.form}</div>}
      <label className="field--full">Título del evento<input name="title" value={values.title} onChange={update} placeholder="Ej. Liderar cuando el marcador va en contra" />{errors.title && <small className="field-error">{errors.title}</small>}</label>
      ...
      <label>Categoría<select name="category" value={values.category} onChange={update}>{['Liderazgo','Resiliencia','Equipo','Rendimiento','Innovación','Bienestar','Otros'].map((item) => <option key={item}>{item}</option>)}</select></label>
      <label>Aforo<input type="number" name="capacity" min="1" max="10000" value={values.capacity} onChange={update} /></label>
      <label className="field--full">Descripción<textarea name="description" value={values.description} onChange={update} rows="6" placeholder="Qué aprenderá el público, quién dará la charla y a quién va dirigida…" />{errors.description && <small className="field-error">{errors.description}</small>}<small>{values.description.length}/1200</small></label>
