...
export const EventDetailPage = () => {
  ...
  return <article className="detail-page">
    ...
    <div className="shell detail-hero"><div className={`detail-visual ${event.poster ? '' : 'detail-visual--empty'}`}>{event.poster ? <img src={event.poster} alt={t("Cartel de {title}", { title: content.title })} /> : <span>{t(event.category)}</span>}</div><div className="detail-summary"><span className="tag">{t(event.category)}</span><h1>{content.title}</h1><EventSpeaker event={event} /><p className="detail-meta"><CalendarDays /> <span>{formatDate(event.date, language)} · {new Intl.DateTimeFormat(locale, {
              hour: '2-digit',
              minute: '2-digit'
            }).format(new Date(event.date))}</span></p><p className="detail-meta"><MapPin /> <span>{event.location}</span></p><p className="detail-meta"><Users /> <span>{t("{count} de {capacity} plazas confirmadas", { count: event.attendees.length, capacity: event.capacity })}</span></p><div className="detail-actions"><button className={attending ? 'button button--confirmed' : 'button button--accent'} onClick={toggle} disabled={actionLoading || previewMode}>{previewMode ? t("Reservas próximamente") : actionLoading ? <><span className="mini-spinner" /> {t("Actualizando…")}</> : attending ? <><Check /> {t("Plaza confirmada")}</> : t("Confirmar asistencia")}</button><button className="icon-button icon-button--border" onClick={share} aria-label={t("Compartir evento")}><Share2 /></button></div>{attending && <p className="confirmation-note">{t("Ya estás en la lista. Puedes cancelar pulsando de nuevo.")}</p>}</div></div>
    <div className="shell detail-content"><section><p className="kicker">{t("Inside the talk")}</p><h2>{t("Lo que te llevarás")}</h2><p className="detail-description">{content.description}</p><div className="creator"><div className="avatar">{event.creator.avatar ? <img src={event.creator.avatar} alt="" /> : <UserRound />}</div><div><small>{t("Experiencia creada por")}</small><strong>{event.creator.name}</strong></div></div></section><aside><p className="kicker">{t("The roster")}</p><h2>{t("El equipo")}</h2>{event.attendees.length ? <ul className="attendee-list">{event.attendees.map(attendee => <li key={attendee._id}><span className="avatar avatar--small">{attendee.avatar ? <img src={attendee.avatar} alt="" /> : attendee.name.slice(0, 1)}</span>{attendee.name}</li>)}</ul> : <p className="muted">{t("Sé la primera persona en entrar al campo.")}</p>}</aside></div>
